  ...
  return (
    <Link to={'/course/' + course._id} onClick={()=> scrollTo(0,0,)} className='border border-gray-500/30 pb-6 overflow-hidden rounded-lg'>
        ...
        <div className='p-3 text-left'>
          ...
          <p className='text-gray-500'>harsh</p>
          <div className='flex items-center space-x-2'>
            <p>{caculateRating(course)}</p>

            <div className='flex'>
              {[...Array(5)].map((_,i)=>( <img className='w-3.5 h-3.5' key={i} src={i<Math.floor(caculateRating(course)) ? assets.star : assets.star_blank}  alt=''/> ))}
            </div>
