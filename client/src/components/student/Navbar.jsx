...
const Navbar = () => {
  ...
  const isCourseListPage = location.pathname.includes('/course-list'); // ✅ fix path
  const { openSignIn } = useClerk()
  const { user } = useUser()

  return (
    <div className={`flex items-center justify-between px-4 sm:px-10 md:px-14 lg:px-36 border-b border-gray-500 py-4 bg ${isCourseListPage ? 'bg-white' : 'bg-cyan-100/70'}`}>
